...
export async function GET() {
  try {
    ...
    if (!res.ok) throw new Error("Failed to fetch powerball.com");
    ...
    // Parse next drawing date from data-drawdateutc attribute
    const drawDateMatch = html.match(/data-drawdateutc="([^"]+)"/);
    const drawDateUTC = drawDateMatch?.[1] ?? null;

    // Parse next drawing label (e.g. "Mon, Apr 6, 2026")
    // It appears inside the #next-drawing div
