...
export default function MegaMillionsCalculator() {
  ...
  function getJackpotLabel(drawDateUTC: string | null): string {
    if (!drawDateUTC) return "Next";
    const drawDate = new Date(drawDateUTC);
    const now = new Date();
    const isToday =
      drawDate.toLocaleDateString("en-US", { timeZone: "America/New_York" }) ===
      now.toLocaleDateString("en-US", { timeZone: "America/New_York" });
    if (isToday) return "Tonight's";
    const day = drawDate.toLocaleDateString("en-US", {
